...
def check_bipartite(edge_list):
    ...
    for e in list(map(set, edge_list)):
        if e <= red or e <= blue:
            return -1
...
def main():
    ...
    # Read the input data.
    with open('rosalind_bip.txt') as input_data:
        input_data.readline()  # Ignore first two lines.
        input_data.readline()
        edge_lists = [edges.split('\n') for edges in input_data.read().strip().split('\n\n')]
        edge_lists = [[[int(node) for node in nodes.split()] for nodes in edges] for edges in edge_lists]

    # Check each edge list to see if it is bipartite.
    bipartite = [check_bipartite(edge_list) for edge_list in edge_lists]
    print(bipartite)
